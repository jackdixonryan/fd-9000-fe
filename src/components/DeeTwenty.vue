<template>
  <div>
    ...
    <button class="randomize" @click="randomFace">Roll!</button>
...
<script>
  export default {
    ...
    watch: { 
      currentFace(newVal, oldVal) {
        const oldDisplayFaceClass= `face-${oldVal}`
        const newDisplayFaceClass = `face-${newVal}`;
        const oldElement = document.getElementsByClassName(oldDisplayFaceClass)[0];
        const newElement = document.getElementsByClassName(newDisplayFaceClass)[0];
    ...
    methods: {
      randomFace() {
        ...
        this.rollTo(this.currentFace, face);
        this.currentFace = face;
      },
      rollTo(oldFace, newFace) { 
        clearTimeout(this.timeoutId);
        const oldDisplayFaceClass= `face-${oldFace}`
        const newDisplayFaceClass = `face-${newFace}`;
        const oldElement = document.getElementsByClassName(oldDisplayFaceClass)[0];
        const newElement = document.getElementsByClassName(newDisplayFaceClass)[0];
        oldElement.classList.remove("active");
        newElement.classList.add("active");
        ...
        const attr = die.setAttribute("data-face", newFace);
        clearTimeout(this.timeoutId);
